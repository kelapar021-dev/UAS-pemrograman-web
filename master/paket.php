...
// 1. Deteksi kolom ID otomatis (Biar gak salah nama kolom lagi)
$cek = mysqli_query($koneksi, "SELECT * FROM paket LIMIT 1");
...
if(isset($_GET['edit'])){
    $id_e = $_GET['edit'];
    $ambil_e = mysqli_query($koneksi, "SELECT * FROM paket WHERE $pk = '$id_e'");
...
if(isset($_POST['simpan'])){
    ...
    if($id_post != ""){
        mysqli_query($koneksi, "UPDATE paket SET " . implode(", ", $update_sql) . " WHERE $pk = '$id_post'");
    } else {
        mysqli_query($koneksi, "INSERT INTO paket (" . implode(", ", $kolom_sql) . ") VALUES (" . implode(", ", $nilai_sql) . ")");
    }
...
if(isset($_GET['hapus'])){
    $id_h = $_GET['hapus'];
    mysqli_query($koneksi, "DELETE FROM paket WHERE $pk = '$id_h'");
    header("location: $halaman_ini");
    exit();
}
?>
...
<html>
<head>
    <title>SUNSET SPORT - DATA PAKET</title>
    ...
    <style>
        ...
        .btn-aksi { padding: 6px 12px; border-radius: 5px; text-decoration: none; color: white; font-size: 12px; font-weight: bold; }
        .badge-harga { color: #2e7d32; font-weight: bold; }
    </style>
</head>
<body>
...
<div class="card">
    <div class="header">
        <h2 style="margin:0; color:#e65100;"><i class="fas fa-box"></i> MASTER DATA PAKET</h2>
            ...
            <div class="input-grid">
                ...
                foreach($fields as $f){
                    if($f->name != $pk){ 
                        ...
                        $type = (strpos($f->name, 'harga') !== false) ? "number" : "text";
                        echo "<input type='$type' name='$f->name' placeholder='Masukkan ".ucwords(str_replace('_',' ',$f->name))."...' value='$val' required>";
                    }
                }
                ?>
                <button type="submit" name="simpan" class="btn-simpan">
                    <i class="fas fa-save"></i> <?php echo $data_e ? "UPDATE PAKET" : "TAMBAH PAKET"; ?>
                </button>
                <?php if($data_e): ?>
    ...
    <div class="table-area">
        <table>
            ...
            <tbody>
                <?php
                $res = mysqli_query($koneksi, "SELECT * FROM paket");
                while($row = mysqli_fetch_assoc($res)){
                    echo "<tr>";
                    foreach($fields as $f) { 
                        $isi = $row[$f->name];
                        if(strpos($f->name, 'harga') !== false) $isi = "Rp " . number_format($isi);
                        echo "<td>".$isi."</td>"; 
                    }
                    ?>
                    <td align="center">
                        <a href="?edit=<?php echo $row[$pk]; ?>" class="btn-aksi" style="background:#0277bd;"><i class="fas fa-edit"></i> EDIT</a>
                        <a href="?hapus=<?php echo $row[$pk]; ?>" class="btn-aksi" style="background:#c62828;" onclick="return confirm('Hapus paket ini?')"><i class="fas fa-trash"></i> HAPUS</a>
                    </td>
                    <?php echo "</tr>";
                } ?>
            </tbody>
